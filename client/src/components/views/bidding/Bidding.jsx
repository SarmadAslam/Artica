import { useState, useEffect, useCallback } from "react";
import Dropdown from "../../globalComponents/Dropdown";
import BiddingCard from "./BiddingCard";
import BidDetailModal from "./BidDetailModal";
import BidConfirmModal from "./BidConfirmModal";
import axios from "axios";
import Loader from "../../loader/Loader";
import endpoints from "../../../constraints/apiConfig";
import imagePath from "../../../constraints/imagepath";
import { toast } from "react-toastify";
import Slider from "rc-slider";
import "rc-slider/assets/index.css";

const Bidding = () => {
  const [selectedCategory, setSelectedCategory] = useState("All Categories");
  const [selectedTag, setSelectedTag] = useState("All Tags");
  const [sortOption, setSortOption] = useState("Newest");
  const [priceRange, setPriceRange] = useState([0, 1000]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isBidConfirmOpen, setIsBidConfirmOpen] = useState(false);
  const [selectedArtwork, setSelectedArtwork] = useState(null);
  const [artworks, setArtworks] = useState([]);
  const [filteredArtworks, setFilteredArtworks] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [bidDetails, setBidDetails] = useState([]);
  const [searchQuery, setSearchQuery] = useState("");
  const [debouncedQuery, setDebouncedQuery] = useState("");
  const [maxPrice, setMaxPrice] = useState(1000);

  const bidOptions = ["All Categories", "Landscape", "Abstract", "Portrait"];
  const bidTags = ["All Tags", "Popular", "Trending", "Ending Soon"];
  const sortOptions = ["Newest", "Highest Bid", "Ending Soon"];

  // Debounce search input
  useEffect(() => {
    const timer = setTimeout(() => {
      setDebouncedQuery(searchQuery);
    }, 300);

    return () => {
      clearTimeout(timer);
    };
  }, [searchQuery]);

  // Fetch artworks from API and determine max price
  useEffect(() => {
    const fetchArtworks = async () => {
      try {
        setLoading(true);
        const response = await axios.get(endpoints.getAllArtsWork);

        if (response.data.success) {
          const artworksWithTimeLeft = response.data.data.map(artwork => {
            const timeLeft = calculateTimeLeft(artwork.expiresAt);
            return {
              ...artwork,
              currentBid: artwork.currentBid || 0,
              startingBid: artwork.startingBid || 0,
              timeLeft
            };
          });
          
          // Calculate max price from artworks with proper fallbacks
          const maxArtworkPrice = Math.max(
            ...artworksWithTimeLeft.map(artwork => 
              artwork.currentBid || artwork.startingBid || 0
            ),
            1000 // default if no artworks
          );
          
          setMaxPrice(Math.ceil(maxArtworkPrice / 100) * 100); // Round up to nearest 100
          setPriceRange([0, Math.ceil(maxArtworkPrice / 100) * 100]);
          
          setArtworks(artworksWithTimeLeft);
          setFilteredArtworks(artworksWithTimeLeft);
        } else {
          setError("Failed to fetch artworks");
        }
      } catch (err) {
        console.error(err);
        setError("An error occurred while fetching artworks");
        toast.error("Failed to load artworks");
      } finally {
        setLoading(false);
      }
    };

    fetchArtworks();
  }, []);

  // Calculate time left for bidding
  const calculateTimeLeft = useCallback((expiresAt) => {
    if (!expiresAt) return { days: 0, hours: 0 };
    
    const difference = new Date(expiresAt) - new Date();
    if (difference <= 0) return { days: 0, hours: 0 };
    
    return {
      days: Math.floor(difference / (1000 * 60 * 60 * 24)),
      hours: Math.floor((difference / (1000 * 60 * 60)) % 24)
    };
  }, []);

  // Format time left display
  const formatTimeLeft = useCallback((timeLeft) => {
    if (!timeLeft) return "Ended";
    if (timeLeft.days > 0) return `${timeLeft.days}d ${timeLeft.hours}h`;
    if (timeLeft.hours > 0) return `${timeLeft.hours}h`;
    return "Ended";
  }, []);

  // Filter and sort artworks
  useEffect(() => {
    if (artworks.length === 0) return;

    let result = [...artworks];

    // Apply search filter with better space handling
    if (debouncedQuery) {
      const queries = debouncedQuery
        .trim()
        .toLowerCase()
        .split(/\s+/)
        .filter(q => q.length > 0);

      if (queries.length > 0) {
        result = result.filter(artwork => {
          const searchFields = [
            artwork.title?.toLowerCase() || "",
            artwork.description?.toLowerCase() || "",
            artwork.category?.toLowerCase() || "",
            artwork.artist?.name?.toLowerCase() || ""
          ];

          return queries.every(q => 
            searchFields.some(field => field.includes(q))
          );
        });
      }
    }

    // Filter by category
    if (selectedCategory !== "All Categories") {
      result = result.filter(artwork => artwork.category === selectedCategory);
    }

    // Filter by tag
    if (selectedTag !== "All Tags") {
      if (selectedTag === "Ending Soon") {
        result = result.filter(artwork => 
          (artwork.timeLeft?.days || 0) < 1 && (artwork.timeLeft?.hours || 0) < 24
        );
      }
      // Add other tag filters as needed
    }

    // Filter by price range
    result = result.filter(artwork => {
      const currentBid = artwork.currentBid || artwork.startingBid || 0;
      return currentBid >= priceRange[0] && currentBid <= priceRange[1];
    });

    // Sort artworks
    switch (sortOption) {
      case "Newest":
        result.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
        break;
      case "Highest Bid":
        result.sort((a, b) => (b.currentBid || b.startingBid || 0) - (a.currentBid || a.startingBid || 0));
        break;
      case "Ending Soon":
        result.sort((a, b) => {
          const aTime = new Date(a.expiresAt)?.getTime() || 0;
          const bTime = new Date(b.expiresAt)?.getTime() || 0;
          return aTime - bTime;
        });
        break;
      default:
        break;
    }

    setFilteredArtworks(result);
  }, [artworks, debouncedQuery, selectedCategory, selectedTag, sortOption, priceRange]);

  const handleBidNowClick = async (artwork) => {
    try {
      setLoading(true);
      setSelectedArtwork(artwork);
      
      const response = await axios.get(`${endpoints.getArtworkBids}/${artwork._id}`);
      setBidDetails(response.data.data || []);
      
      setIsModalOpen(true);
    } catch (err) {
      console.error("Error handling bid click:", err);
      toast.error("Failed to load bid details");
    } finally {
      setLoading(false);
    }
  };

  const handleOpenBidConfirm = () => {
    setIsModalOpen(false);
    setIsBidConfirmOpen(true);
  };

  const handleBidSuccess = (newBidAmount) => {
    setArtworks(prevArtworks => 
      prevArtworks.map(artwork => 
        artwork._id === selectedArtwork._id 
          ? { ...artwork, currentBid: newBidAmount } 
          : artwork
      )
    );
    toast.success("Bid placed successfully!");
    setIsBidConfirmOpen(false);
  };

  if (loading && artworks.length === 0) {
    return <Loader />;
  }

  if (error) {
    return <div className="text-center text-red-500 p-8">{error}</div>;
  }

  return (
    <div className="min-h-screen">
      {/* Filter Section */}
      <div className="py-6 bg-[#F35E21] text-white sticky top-0 z-10">
        <div className="w-[90%] mx-auto flex flex-wrap items-center gap-4">
          {/* Search Input */}
          <div className="relative flex-grow max-w-md">
            <input
              type="text"
              placeholder="Search artworks by title, description, artist or category..."
              className="w-full p-2 pl-10 bg-white text-black rounded-lg"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            <svg
              className="absolute left-3 top-3 h-5 w-5 text-gray-400"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"
              />
            </svg>
          </div>
          
          <Dropdown 
            defaultOption="All Categories" 
            options={bidOptions} 
            selectedValue={selectedCategory} 
            onChange={setSelectedCategory} 
            className="min-w-[180px]"
          />
          
          {/* Price Range Slider */}
          <div className="flex-grow max-w-xs">
            <div className="bg-white p-3 rounded-lg">
              <div className="text-black text-sm mb-1">
                Price Range: ${priceRange[0]} - ${priceRange[1]}
              </div>
              <Slider
                range
                min={0}
                max={maxPrice}
                step={10}
                defaultValue={[0, maxPrice]}
                value={priceRange}
                onChange={(value) => setPriceRange(value)}
                trackStyle={[{ backgroundColor: "#F35E21" }]}
                handleStyle={[
                  { backgroundColor: "#F35E21", borderColor: "#F35E21" },
                  { backgroundColor: "#F35E21", borderColor: "#F35E21" }
                ]}
                railStyle={{ backgroundColor: "#e5e7eb" }}
              />
            </div>
          </div>
          
          <Dropdown 
            defaultOption="All Tags" 
            options={bidTags} 
            selectedValue={selectedTag} 
            onChange={setSelectedTag} 
            className="min-w-[120px]"
          />
          
          {sortOptions.map(option => (
            <button
              key={option}
              className={`py-2 px-4 rounded-lg ${
                sortOption === option 
                  ? "bg-white text-[#F35E21] font-bold" 
                  : "bg-transparent border border-white"
              }`}
              onClick={() => setSortOption(option)}
            >
              {option}
            </button>
          ))}
        </div>
      </div>

      {/* Artworks Grid */}
      <div className="w-[90%] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 py-12">
        {filteredArtworks.length > 0 ? (
          filteredArtworks.map((artwork) => (
            <BiddingCard 
              key={artwork._id} 
              artwork={{
                _id: artwork._id,
                img: artwork.images?.[0] ? `${imagePath}${artwork.images[0]}` : '',
                title: artwork.title || 'Untitled',
                description: artwork.category || 'No category',
                price: `$${(artwork.currentBid || artwork.startingBid || 0).toLocaleString()}`,
                timeLeft: formatTimeLeft(artwork.timeLeft),
                isEndingSoon: (artwork.timeLeft?.days || 0) < 1 && (artwork.timeLeft?.hours || 0) < 24
              }} 
              onBidNowClick={() => handleBidNowClick(artwork)} 
            />
          ))
        ) : (
          <div className="col-span-full text-center py-12">
            <p className="text-xl text-gray-500">
              {debouncedQuery ? 
                "No artworks found matching your search criteria" : 
                "No artworks available"}
            </p>
          </div>
        )}
      </div>

      {/* Modals */}
      {selectedArtwork && (
        <>
          <BidDetailModal 
            isOpen={isModalOpen} 
            onClose={() => setIsModalOpen(false)} 
            selectedArtwork={selectedArtwork} 
            onBidClick={handleOpenBidConfirm}
            bidDetails={bidDetails}
          />

          <BidConfirmModal 
            isOpen={isBidConfirmOpen} 
            onClose={() => setIsBidConfirmOpen(false)} 
            selectedArtwork={{
              img: selectedArtwork.images?.[0] || '',
              title: selectedArtwork.title || 'Untitled',
              price: selectedArtwork.currentBid || selectedArtwork.startingBid || 0
            }}
            artworkId={selectedArtwork._id}
            currentHighestBid={selectedArtwork.currentBid || selectedArtwork.startingBid || 0}
            minIncrement={selectedArtwork.minIncrement || 10}
            onBidSuccess={handleBidSuccess}
          />
        </>
      )}
    </div>
  );
};

export default Bidding;